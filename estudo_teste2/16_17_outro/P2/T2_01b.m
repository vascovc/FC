clear all, close all, clc

% Constantes do sistema
D = 0.02;
m = 5;
L = 1;

% Pré-alocações
x0 = 0;
xf = L;
t0 = 0;
tf = 3;

dx = 0.01;
dt = 0.001;

x = x0:dx:xf;
t = t0:dt:tf;

Nx = length(x);
Nt = length(t);

u = zeros(Nx,Nt);

% Condições iniciais
u(:,1) = 0;

% Condições fronteira
u(1,:) = 0;
u(Nx,:) = 1;

% Método de Euler
for j = 1:Nt-1
    for i = 2:Nx-1
        u(i,j+1) = u(i,j) + dt*(D*(u(i-1,j) - 2*u(i,j) + u(i+1,j))/dx^2 + m*u(i,j)*(1-u(i,j)));
    end
   
%     plot(x,du(:,j))
%     pause(0.1)
end

figure(1)
mesh(t,x,u)
xlabel('t')
ylabel('x')
zlabel('u')
figure(2)
contourf(x,t,u')
colorbar
xlabel('x')
ylabel('t')

% Olhando para o contourplot vemos que entre t = 0 e t = 1.5 (nos cálculos usei antes 0.5 e 1) existem bandas
% paralelas da mesma cor, o que significa que entre esses pontos, estas
% bandas paralelas viajam a velocidades constantes


[row, col] = find(u > 0.1-0.001  & u <0.1 + 0.001);
figure(3)
plot(t(col(50:200)),x(row(50:200)))
xlabel('t')
ylabel('x')
p = polyfit(t(col(50:200)),x(row(50:200)),1);

vel = abs(p(1));
v_analitica = 2*sqrt(D*m);

erro = (abs(v_analitica-vel)/v_analitica)*100; % erro = 13.5662%
